# [JavaScript QA Engineer OTUS](https://otus.ru/lessons/qajs/)

## Структура курса

### Введение в автоматизацию тестирования на JS
* [Введение в автоматизацию тестирования](./lessons/introduction.md) ✍️
* [Git на кончиках пальцев](./lessons/git.md) ✍️
* [Настройка окружения](./lessons/env.md) ✍️
* [Основы JS. Функции](./lessons/js-functions.md) ✍️
* [Unit тесты и data driven testing](./lessons/unit-test.md) ✍️

### Тестирование API
* [Препарируем HTTP запросы](./lessons/http.md) ✍️
* [Основы JS. Объекты и массивы](./lessons/js-object.md) ✍️
* [Библиотеки для тестирования API](./lessons/api-library.md) ✍️
* [Шаблоны проектирования в тестировании API](./lessons/api-patterns.md) ✍️
* [Валидируем HTTP запросы](./lessons/api-validation.md) ✍️
* [Репортеры и внешние сервисы](./lessons/reporters.md) ✍️
* [TypeScript в тестировании](./lessons/typescript.md) ✍️

### Практики CI/CD
* [Стратегия автоматизации в agile проектах и планирование покрытия](./lessons/strategy.md)
* [Контейнеры. Docker](./lessons/docker.md) ✍️
* [Катит - не катит. Github CI часть 1](./lessons/ci-1.md)
* [Катит - не катит. Github CI часть 2](./lessons/ci-2.md) ✍️
* [Q&A сессия](./qa.md)
* [SQL](./sql.md)

### Тестирование UI
* [Обзор инструментов для тестирования UI](./lessons/ui-test.md)
* [Локаторы](./lessons/locators.md) ✍️
* [Основные сценарии использования Playwright](./lessons/playwright.md) ✍️
* [Шаблоны проектирования и техники тест-дизайна в тестировании UI](./lessons/ui-patterns.md) ✍️
* [Основы JS. Классы](./lessons/js-classes.md)
* [Основы JS. Классы в типовых сценариях автотестов](./lessons/test-classes.md) ✍️
* [Behavior Driven Development и acceptance тесты](./lessons/bdd.md) ✍️

### Проектная работа
* Выбор темы и организация проектной работы
* Консультация по проектам и домашним заданиям
* Защита проектных работ
