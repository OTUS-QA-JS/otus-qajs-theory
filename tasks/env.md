...
* создайте файл: `.prettierrc` с таким содержимым:
```json
{
    "semi": false,
    "singleQuote": true,
...
```
* создайте файл: `.prettierignore` с таким содержимым:
```
reports
build
coverage
...
* вы можете настроить `prettier` под ваши предпочтения, смотрите совет #1 ниже
...
---
## Критерии оценивания
* Линтер или форматтер настроен и работает корректно.
* Скрипты в package.json выполняют свои функции.
* Код соответствует стандартам, установленным линтером и форматтером.